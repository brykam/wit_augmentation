...
def train_model(X, y, masks,
    ...
    for train, valid in kfold.split(X_train_valid, y_train_valid):
        ...
        X_aug, y_aug = augment_dataset(
            X_train, y_train, mode=augmentation, masks=masks_train)
    ...
    save_results(
        directory + f'kb_{model_name}_{epochs}_{augmentation}_results.txt',
        acc_per_fold, loss_per_fold)
    save_plots(
        directory + f'kb_{model_name}_{epochs}_{augmentation}_plots.png', 
        histories)
...
if __name__ == "__main__":
    ...
    model_names = ['inception'] # 'vgg16', 'vgg19', 
