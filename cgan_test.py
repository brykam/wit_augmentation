...
def train(models, data, params):
    ...
    # network parameters
    batch_size, latent_size, train_steps, num_labels, model_name, cats = params
...
def plot_images(generator,
    ...
    labels = np.argmax(noise_class, axis=1)
...
def build_and_train_models():
    ...
    x_train, y_train = helpers.get_smear_set(with_labels=True)
    width, height = 128, 128
    for i, image in enumerate(x_train):
        x_train[i] = cv2.resize(image, (height, width))
    ...
    onehot_y = onehot.fit_transform(y_train)
    y_train = onehot_y.toarray()

    num_labels = len(onehot.categories_[0])
    # y_train = to_categorical(y_train)

    model_name = "cgan_smear"
    # network parameters
    ...
    params = (batch_size, latent_size, train_steps, num_labels, model_name, onehot.categories_[0])
