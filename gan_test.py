...
def build_and_train_models():
    ...
    X_train, labels = helpers.get_smear_set()
    X_canc = X_train[len(X_train) - sum(labels.ravel()):]
    width, height = 128, 128
    for i, image in enumerate(X_canc):
        X_canc[i] = cv2.resize(image, (height, width))
    x_train = np.asarray(X_canc)
    # reshape data for CNN as (28, 28, 1) and normalize
    image_size = x_train.shape[1]
    x_train = np.reshape(x_train, [-1, image_size, image_size, 3])
    x_train = x_train.astype('float32') / 255

    model_name = "dcgan_smear"
    # network parameters
